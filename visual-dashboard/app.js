...
async function refreshAll({ silent = false } = {}) {
  ...
  // Only the dedicated health endpoint determines service availability. A
  // transient failure in one data panel must not make the whole service look offline.
  const healthResult = results[0];
  const online = healthResult.status === 'fulfilled';
  const partialFailure = results.slice(1).some(result => result.status === 'rejected');
  setServiceState(online, new Date());
  if (online && !silent) {
    showToast(partialFailure ? '服务在线，部分数据面板暂时未刷新' : '数据已刷新');
  }
  if (!online && !silent) showToast('后端健康检查失败，请稍后重试');
}
